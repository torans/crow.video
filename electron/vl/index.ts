...
- "colors": 产品主体所有可见颜色的数组，用英文小写（最多4个）。这是关键：你必须提取产品主体的所有不同颜色，不能只提取最主要的。例如，黑色手机戴着绿色外壳 → ["black", "green"]。红色杯子带白色盖子 → ["red", "white"]。白色产品带黑色标签 → ["white", "black"]。排除：装饰图案、刺绣、logo、印刷图形、花卉装饰、纽扣配件、背景、阴影、环境色。
...
示例：黑色手机戴着绿色外壳 → ["black", "green"]。红色杯子带白色盖子 → ["red", "white"]。
返回 JSON：description（中文，最多100字）、colors（最多4个英文小写主体颜色）、tags（中文物品标签，最多5个）。`

const FRAME_ANALYSIS_PROMPT = `分析这个视频帧。目标是为短视频自动剪辑提供“镜头语义”。
只提取产品/前景物体的主要材质颜色（最多4个）。排除：装饰图案、文字、背景、阴影、环境色。
示例：红色衬衫带白色波点 → colors 只写 ["red"]。

description 必须用中文简短写清：
1. 画面主体是什么（人/物/场景）
2. 镜头类型（整体展示 / 局部特写 / 实操演示 / 效果对比）
3. 画面中的动态变化（如“正在倾倒液体”、“特写手指触摸”、“镜头平移展示”）

tags 最多 5 个，【必须】优先包含 1-2 个“镜头结构标签”，只能从以下词中选择最贴切的：
- 吸睛
- 场景
- 产品
- 细节
...
剩余 tags 再补充具体的物品或场景词，例如：户外、居家、水杯、手机、桌面、运动、服饰等。

打标原则（严格遵守）：
- 强动作、强冲击、大特写、高光瞬间 => 吸睛
- 真实使用、操作过程、展示功能的环境 => 场景
- 产品整体清晰可见、主体明确 => 产品
- 近距离局部特写、材质/接口/做工 => 细节
- 使用前后对比、结果反馈 => 效果
- 适合片尾引导、品牌Logo收尾 => 转化

返回纯 JSON：{"description": "中文场景描述", "colors": ["..."], "tags": ["..."], "appeal": 8}。
appeal 为1-10整数：画面作为短视频素材的“可用性与吸引力”。8-10分代表高质量特写/戏剧性动作；6-7分代表清晰的使用演示；3-5分代表静态平铺展示；1-2分代表模糊/黑屏/无意义过渡帧。`
