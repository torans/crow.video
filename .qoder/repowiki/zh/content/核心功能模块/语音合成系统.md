...
## 更新摘要
**变更内容**
- 新增ElevenLabs TTS引擎集成，实现混合语音合成系统
- 新增ElevenLabsTtsPanel组件，提供完整的语音选择器和实时预览功能
- 更新TTS控制组件，支持双引擎切换和配置管理
- 扩展IPC层，新增ElevenLabs相关API处理函数
- 更新应用状态管理，新增ElevenLabs配置和语音列表存储
...
- [src/components/tts/ElevenLabsTtsPanel.vue:1-510](file://src/components/tts/ElevenLabsTtsPanel.vue#L1-L510)
...
- [electron/tts/index.ts:1-213](file://electron/tts/index.ts#L1-L213)
- [electron/lib/edge-tts.ts:1-632](file://electron/lib/edge-tts.ts#L1-L632)
- [electron/lib/elevenlabs-tts.ts:1-357](file://electron/lib/elevenlabs-tts.ts#L1-L357)
...
## 核心组件
- **EdgeTTS引擎封装**：负责与微软语音服务建立WebSocket连接、发送SSML请求、接收音频流与词边界元数据、拼接音频并生成SRT字幕。
- **ElevenLabs引擎封装**：负责与ElevenLabs API交互，支持多模型选择、高级参数配置（稳定性、相似度、风格等）、实时语音合成与字幕生成。
...
- [electron/lib/elevenlabs-tts.ts:201-357](file://electron/lib/elevenlabs-tts.ts#L201-L357)
- [electron/tts/index.ts:148-213](file://electron/tts/index.ts#L148-L213)
- [electron/ipc.ts:191-209](file://electron/ipc.ts#L191-L209)
- [src/views/Home/components/TtsControl.vue:20-421](file://src/views/Home/components/TtsControl.vue#L20-L421)
- [src/components/tts/ElevenLabsTtsPanel.vue:1-510](file://src/components/tts/ElevenLabsTtsPanel.vue#L1-L510)
...
Panel->>Preload : 调用 window.electron.elevenlabsTtsSetApiKey()
...
- **模型支持**
  - eleven_v3：表现力最强，支持70+语言，适合精细调优。
  - eleven_multilingual_v2：多语言模型，中文表现稳定。
  - eleven_flash_v2_5：低延迟模型，适合对话场景。
...
**图表来源**
- [electron/lib/elevenlabs-tts.ts:201-357](file://electron/lib/elevenlabs-tts.ts#L201-L357)
- [electron/lib/elevenlabs-tts.ts:71-199](file://electron/lib/elevenlabs-tts.ts#L71-L199)
...
ElevenParams --> ElevenSynthesize["调用 ElevenLabsTTS.synthesize()"]
...
UI->>Preload : window.electron.elevenlabsTtsGetVoiceList(filters)
...
- **EdgeTTS参数**
  ...
  - 性别设置：支持Male/Female，与语音列表的Gender字段匹配。
...
## 结论
该语音合成系统现已升级为混合语音合成系统，集成了EdgeTTS和ElevenLabs两大引擎，提供了更丰富的语音选择和更高质量的语音合成能力。系统通过清晰的分层架构实现了从UI控件到主进程IPC、再到引擎封装的完整链路。ElevenLabs引擎支持多模型选择、高级参数配置、实时语音预览等功能，而EdgeTTS引擎保持原有的免费优势和字幕生成功能。系统在易用性与稳定性之间取得了良好平衡，为短视频渲染阶段提供了可靠的语音合成解决方案。
