...
I --> J
J --> K["映射输出流 vout+aout"]
K --> L["编码: libx264/aac<br/>固定fps/分辨率/进度输出"]
```
...
## 结论
该视频渲染引擎以 Electron 为载体，结合 FFmpeg 的强大滤镜链能力，实现了从多轨道视频拼接到字幕嵌入、音频混合与响度归一化的完整流水线。通过清晰的渲染状态机、稳定的 IPC 通信与进度回传、完善的错误处理与中断恢复机制，为上层 UI 提供了可靠的渲染能力。配合 SQLite 数据持久化与 TTS 字幕生成，形成从素材准备到成品输出的一体化解决方案。未来可在 GPU 加速、更丰富的滤镜链与参数调节方面进一步扩展。
